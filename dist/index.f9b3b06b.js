...
})({"iHFng":[function(require,module,exports,__globalThis) {
...
(()=>{
    ...
    const openModal = (event)=>{
        ...
        if (targetKey) {
            ...
            if (modal) modal.classList.remove("is-hidden"); // Show the modal
        }
    };
    // Function to handle modal closing
    const closeModal = (event)=>{
        const modal = event.currentTarget.closest(".modal, .login, .review, .buynow, .buynow2, .subscribe");
        if (modal) modal.classList.add("is-hidden"); // Hide the modal
    };
    // Function to close modal when clicking outside the modal content
    const closeModalOnOutsideClick = (event)=>{
        if (event.target.classList.contains("modal") || event.target.classList.contains("login") || event.target.classList.contains("review") || event.target.classList.contains("buynow") || event.target.classList.contains("buynow2") || event.target.classList.contains("subscribe")) event.target.classList.add("is-hidden");
    };
...
},{}]},["iHFng","aHHgN"], "aHHgN", "parcelRequire94c2")
